...
## Автоматическое создание сервисного аккаунта для Kubernetes

- При запуске Terraform сервисный аккаунт для управления Kubernetes-кластером создаётся автоматически, вручную создавать его не требуется.
- Все необходимые IAM-права (`k8s.admin` для кластера, `container-registry.editor` для docker registry) назначаются автоматически.
- Дальнейшие ресурсы (кластер, node group) используют этот аккаунт для безопасного и корректного управления ресурсами.
